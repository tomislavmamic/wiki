---
id: classification
title: Classification of Dental Diseases
sidebar_label: Classification
---
  

```
Oral conditions
├── Dental tissue conditions
│   └── Crown
│       └── Dental caries
│           ├── Initial
│           ├── Moderate
│           ├── Severe
│           ├── Arrested
│           └── Odontoclasia
│       └── Tooth wear
│           ├── Erosion
│           ├── Abrasion
│           ├── Attrition
│           └── Abfraction
│   └── Root
│       └── Root caries
│           ├── Initial
│           ├── Moderate
│           ├── Severe
│           └── Arrested
│       └── Hypercementosis
│   └── Pulp
│       ├── Normal pulp
│       ├── Reversible pulpitis
│       ├── Asymptomatic irreversible pulpitis
│       ├── Symptomatic irreversible pulpitis
│       ├── Hyperplastic pulpitis
│       ├── Pulp necrosis
│       ├── Previously treated pulp
│       ├── Previously initiated therapy
│       └── Pulp degeneration
│           ├── Pulpal calcifications
│           ├── Pulpal stones
│           ├── Internal resorption
│           └── Atrophic degeneration
│   └── Periapical tissues
│       ├── Asymptomatic apical periodontitis
│       ├── Symptomatic apical periodontitis
│       ├── Acute apical abscess
│       ├── Chronic apical abscess
│       └── Condensing osteitis
│   └── Developmental anomalies
│       └── Tooth number
│           ├── Anodontia
│           ├── Hypodontia
│           ├── Oligodontia
│           └── Hyperdontia
│   └── Tooth size
│       ├── Microdontia
│       ├── Macrodontia
│       └── Anomalies of the root size
│           ├── Rhizomegaly
│           └── Rhizomicry
│   └── Tooth shape
│       ├── Fusion
│       ├── Gemination
│       ├── Concrescence
│       ├── Dens evaginatus
│       ├── Dens invaginatus
│       ├── Taurodontism
│       ├── Hutchinson s incisors
│       ├── Moon s/mulberry molars
│       └── Anomalies of root shape
│           └── Dilaceration
│   └── Tooth position
│       ├── Transposition
│       ├── Rotation
│       └── Inclination
│   └── Tooth colour
│       └── Intrinsic discolorations
│           └── Due to metabolic disturbances during odontogenesis
│               ├── Discoloration of teeth in alkaptonuria
│               ├── Discoloration of teeth in porphyria
│               ├── Discoloration of teeth in hyperbilirubinemia
│               └── Discoloration of teeth in cystic fibrosis
│           └── Drug induced
│               ├── Tetracyclines
│               └── Fluorine
│           └── Due to traumatic injury
│       └── Extrinsic discoloration
│   └── Tooth eruption
│       ├── Pericoronitis
│       ├── Eruption cyst
│       ├── Epstein pearls
│       ├── Bohn s nodules
│       ├── Dental lamina cyst
│       └── Precocious eruption(Dentitio praecox)
│           ├── Natal teeth
│           └── Neonatal teeth
│       ├── Retarded eruption (Dentitio tarda)
│       ├── Ankylosis
│       ├── Ectopic eruption
│       ├── Impacted tooth
│       └── Embeded tooth
│   └── Tooth structure
│       └── Structural anomalies of enamel
│           ├── Amelogenesis imperfecta
│           └── Enamel hypoplasia
│       └── Structural anomalies of dentine
│           ├── Dentinogenesis imperfecta
│           └── Dentinal dysplasia
│       ├── Structural anomalies of cementum
│       └── Structural anomalies of enamel and dentine
│           └── Regional odontodysplasia
│   └── Traumatic injuries
│       └── Fractures
│           ├── Infraction
│           ├── Enamel fracture
│           ├── Enamel-dentin fracture
│           ├── Enamel-dentin-pulp fracture
│           ├── Crown-root fracture without pulp exposure
│           ├── Crown-root fracture with pulp exposure
│           └── Root fracture
│       └── Luxations
│           ├── Concussion
│           ├── Subluxation
│           ├── Extrusive luxation
│           ├── Intrusive luxation
│           └── Lateral luxation
│       └── Avulsion
├── Soft tissue conditions
│   └── Gingival and periodontal diseases
│       └── Gingival diseases
│           └── Inflammatory gingival diseases
│               ├── Acute gingivitis
│               ├── Chronic gingivitis
│               ├── Plaque induced gingivitis
│               ├── Puberty - associated gingivitis
│               ├── Menstrual cycle - associated gingivitis
│               ├── Pregnancy - associated gingivitis
│               ├── Diabetes mellitus - associated gingivitis
│               ├── Leukemia - associated gingivitis
│               ├── Drug influenced gingivitis
│               ├── Vitamin C deficiency gingivitis
│               ├── Plasma cell gingivitis
│               ├── Necrotising ulcerative gingivitis (NUG)
│               ├── Gingival abscess
│               └── Pericoronitis
│           └── Gingival enlargement
│               ├── Chronic Inflammatory gingival enlargement
│               ├── Drug influenced gingival enlargement
│               └── Pregnancy influenced gingival enlargement (epullis)
│           ├── Gingival recession
│           └── Traumatic injuries
│               ├── Chemical injury
│               ├── Physical injury
│               └── Thermal injury
│           ├── Foreign body reaction
│           └── Carcinoma of the gingiva
│       └── Periodontal diseases
│           ├── Chronic periodontitis
│           └── Aggresive periodontitis
│               ├── Adult
│               └── Juvenile
│           ├── Necrotising ulcerative periodontitis
│           ├── Periodontal abscess
│           └── Periodontitis associated with an endodontic lesion
│   └── Diseases of lips
│       ├── Exfoliative cheilitis
│       ├── Actinic cheilitis
│       ├── Allergic contact cheilitis
│       ├── Angular cheilitis
│       ├── Glandular cheilitis
│       ├── Granulomatous cheilitis (Orofacial Granulomatosis)
│       ├── Eczematous cheilitis
│       ├── Plasma cell cheilitis
│       ├── Macrocheilia
│       ├── Microcheilia
│       ├── Melkersson - Rosenthal syndrome
│       ├── Ascher Syndrome
│       └── Lip carcinoma
│   └── Diseases of tongue
│       └── Anomalies
│           ├── Macroglossia
│           ├── Microglossia
│           ├── Aglossia
│           ├── Diglossia
│           ├── Lingua bifida
│           ├── Lingual struma
│           └── Lingua plicata
│       └── Inflammatory diseases
│           ├── Glossitis areata superficialis ( Geographic tongue)
│           ├── Median rhomboid glossitis
│           ├── Lingua villosa (Hairy tongue)
│           ├── Lingual papillitis
│           ├── Sublingual varices
│           ├── Moller Hunter glossitis
│           └── Granulomatous glossitis
│   └── Infections of oral mucosa
│       └── Bacterial infections
│           ├── Gingivostomatitis ulcerosa necroticans
│           ├── Stomatitis gangrenosa (Noma)
│           ├── Streptococcal gingivostomatitis
│           ├── Oral actinomycosis
│           └── Oral Tuberculosis
│               ├── Primary
│               └── Secondary
│           └── Syphilis
│       └── Viral infections
│           ├── Acute herpetic gingivostomatitis
│           ├── Reccurent herpetic gingivostomatitis
│           ├── Oral herpes zoster
│           ├── Verruca vulgaris (Common Wart)
│           ├── Focal epithelial hyperplasia (Heck disease)
│           ├── Tonsillar hypertrophy due to adenovirus infection
│           ├── Herpangina
│           ├── Hand-foot-and mouth disease
│           └── Epidemic stomatitis
│       └── Fungal infections
│           ├── Acute pseudomembranos candidiasis (Soor)
│           ├── Acute atrophic candidiasis
│           ├── Chronic atrophic candidiasis
│           ├── Chronic hyperplastic candidiasis
│           ├── Mucocutaneous candidiasis
│           ├── Histoplasmosis
│           └── Blastomycosis
│       └── Recurrent aphthous stomatitis
│           ├── Major aphthous ulcerations (Aphthae major)
│           ├── Minor aphthous ulcerations (Aphthae minor)
│           ├── Herpetiform aphthous ulcerations
│           └── Aphthous ulcerations in Behcet s disease
│   └── White lesions
│       ├── Leukoplakia
│       ├── Leukoedema
│       ├── White sponge nevus
│       ├── Frictional hyperkeratosis
│       └── Hyperkeratosis caused by nicotine
│   └── Red lesions
│       └── Erythroplakia
│   └── Traumatic injuries
│       └── Physical injuries
│           ├── Linea alba
│           ├── Chronic biting (Morsicatio buccarum)
│           ├── Reactive fibrous hyperplasia ( Epullis fisuratum)
│           ├── Inflammatory papillary hyperplasia
│           ├── Denture stomatitis
│           ├── Traumatic ulcer
│           └── Traumatic fibroma
│       └── Chemical injuries
│           ├── Chemical burn
│           └── Post anesthetic ulceration
│       └── Radiation injuries
│           ├── Oral mucositis
│           └── Actinic cheilitis
│       ├── Electrical injuries
│       └── Thermal injuries
│   └── Oral manifestations of systemic diseases
│       └── Gastrointestinal diseases
│           ├── Gastroesophageal Reflux Disease (GERB)
│           ├── Ulcerative colitis
│           ├── Chron disease
│           ├── Pyostomatitis vegetans
│           ├── Chronic liver disease
│           └── Hepatitis C
│       └── Nutritional diseases
│           └── Vitamin deficiency
│               ├── Vitamin B
│               ├── Vitamin C
│               ├── Vitamin A
│               ├── Vitamin D
│               └── Vitamin K
│           └── Mineral deficiency
│               ├── Iron
│               └── Zinc
│       └── Hematologic disorders
│           └── White blood cells disorders
│               ├── Leukemia
│               ├── Lymphoma
│               ├── Cyclic neutropenia
│               ├── Langerhans cell histiocytosis ( Histiocytosis X)
│               ├── Multiple myeloma
│               └── Mastocytosis
│           └── Platelet disorders
│               └── Thrombocytopenia
│           └── Red blood cells disorders
│               ├── Anemias
│               ├── Hemochromatosis
│               ├── Congenital erythropoietic porphyria
│               ├── Thalassemias
│                   ├── Sickle cell disease
│                   ├── Polycythemia vera
│       └── Connective tissue disorders
│           ├── Sjogren syndrome
│           └── Kawasaki disease
│       └── Pulmonary conditions
│           ├── Weneger granulomatosis ( granulomatosis with polyangitis)
│           └── Sarcoidosis
│       └── Multisystem conditions
│           └── Amyloidosis
│       ├── HIV
│       └── Cutaneous diseases
│           ├── Psoriasis
│           └── Acanthosis nigricans
│       └── Neurologic disorders
│           └── Neurofibromatosis
│               ├── Type 1
│               └── Type 2
│       └── Endocrine diseases
│           ├── Diabetes
│           ├── Multiple endocrine neoplasia
│           └── Thyroid disorders
│               ├── Hyperthyroidism
│               └── Hypothyroidism
│           └── Parathyroid disorders
│               ├── Hyperparathyroidism
│               └── Hypoparathyroidism
│           └── Adrenal disorders
│               ├── Hypocorticolism
│               └── Hypercorticolism
│       └── Drug induced conditions
│           ├── Drug induced aphtous stomatitis
│           ├── Dry mouth
│           ├── Lichenoid drug reaction
│           ├── Drug induced gingival hyperplasia
│           └── Steroid induced candidiasis
│   └── Oral dermatoses
│       ├── Erythema exudativum multiforme
│       ├── Herpetiform dermatitis ( Duhring)
│       ├── Lichen ruber
│       └── Pemphigus
│           ├── Pemphigus vulgaris
│           ├── Pemphigus foliaceus
│           ├── Pemphigus seborrhoicus
│           ├── Pemphigus vegetans
│           └── Brazilian pemphigus
│       └── Pemphigoid
│           ├── Bulous pemphigoid
│           └── Benign mucosal pemphigoid
│       └── Lupus erythematodes
│           ├── Lupus erythematodes chronicus ( Cutaneous lupus)
│           └── Lupus erythematodes systemicus
│       ├── Sclerodermia
│       └── Epidermolysis bullosa
│   └── Syndromes
│       ├── Papillon - Lefevre syndrome
│       ├── Meleda disease
│       ├── Anhydrotic ectodermal dysplasia
│       ├── Gorlin -Goltz syndrome
│       ├── Sturge-Weber- Krabbe disease
│       ├── Keratosis follicularis ( Darier-White disease)
│       ├── Acrodermatitis enteropathica
│       └── Peutz-Jeghers-Touraine syndrome
│   └── Soft tissue calcifications
│       ├── Salivary calculi
│       ├── Calcified lymph nodes
│       ├── Calcified tonsils
│       ├── Phleboliths
│       └── Calcified acne scars
│   └── Foregin bodies
│       ├── Within soft tissues
│       └── On or overlying the skin
├── Bone tissue conditions
│   └── Radiolucent lesions
│       └── Infections in the jaws
│           └── Osteomyelitis
│               ├── Suppurative osteomyelitis
│               ├── Focal sclerosing osteomyelitis -Condensing osteitis
│               ├── Diffuse sclerosing osteomyelitis
│               └── Proliferative periostitis- Garee s osteomyelitis
│           └── Osteoradionecrosis
│           └── Medication-related osteonecrosis of the jaws
│       └── Cysts
│           └── Odontogenic
│               ├── Radicular cyst
│               ├── Residual radicular cyst
│               ├── Lateral periodontal cyst
│               ├── Dentigerous cyst
│               └── Odontogenic keratocyst
│           └── Nonodontogenic
│               └── Nasopalatine duct/incisive canal cyst
│       └── Tumors or tumor- like lesions
│           └── Odontogenic
│               ├── Ameloblastoma
│               ├── Squamous odontogenic tumor
│               ├── Calcifying epithelial odontogenic tumor (Pindborg tumor)
│               ├── Adenomatoid odontogenic tumor
│               ├── Ameloblastic fibroma
│               ├── Ameloblastic fibro-odontoma
│               ├── Calcifying cystic odontogenic tumor
│               ├── Odontogenic fibroma
│               ├── Odontogenic myxoma
│               ├── Odontogenic carcinoma
│               └── Odontogenic sarcoma
│           └── Nonodontogenic
│               └── Intrinsic primary bone tumors
│                   └── Benign
│                       ├── Fibroma
│                       ├── Chondroma
│                       ├── Central hemangioma
│                       └── Neurofibroma
│                   └── Malignant
│                       ├── Osteosarcoma
│                       ├── Fibrosarcoma
│                       └── Chonrosarcoma
│               └── Extrinsic primary tumors involving bone
│                   └── Squamous cell carcinoma
│               ├── Secondary metastatic bone tumors
│               └── Lymphoreticular tumors of bone
│                   ├── Multiple myeloma
│                   ├── Large cell lymphoma
│                   ├── Burkitt's lymphoma
│                   └── Ewing's tumor
│               └── Langerhans cell disease
│                   ├── Eosinophilic granuloma
│                   ├── Hand-Schuller-Christian's disease
│                   └── Letterer-Siwe disease
│       └── Bony lesions
│           └── Giant cell lesions
│               ├── Central giant cell lesion ( granuloma)
│               ├── Brown tumor in hyperparathyroidism
│               ├── Cherubism
│               └── Aneurysmal bone cyst
│           └── Osseous dysplasia (early stages)
│               ├── Periapical osseous dysplasia
│               ├── Focal osseous dysplasia
│               └── Familial gigantiform cementoma
│           └── Other lesions
│               ├── Osifying fibroma
│               ├── Fibrous dysplasia
│               ├── Simple bone cyst
│               └── Staphne s bone cavity
│       └── Alveolar bone loss
│           ├── Horizontal
│           ├── Vertical
│           └── Furcation involvement
│       └── Localised bone cavities
│           └── Staphne s bone cavity
│       └── Iatrogenic
│           ├── Surgical (fibrous) healing defect
│           └── Post-extraction socket
│   └── Radioopaque lesions
│       └── Abnormalities of the teeth
│           ├── Unerupted teeth
│           ├── Embedded teeth
│           ├── Impacted teeth
│           ├── Supernumerary teeth
│           ├── Hypercementosis
│           └── Rootremnants
│       └── Developmental bony radioopaque lesions
│           ├── Exostoses
│           ├── Mandibular tori
│           └── Palatal tori
│       └── Inflammatory bony radioopaque lesions
│           ├── Sclerosing osteitis
│           └── Osteomyelitis
│               ├── Sequestrum
│               └── Involucrum
│       └── Odontogenic tumors
│           ├── Calcifying epihtelial odontogenic tumor (CEOT)
│           ├── Ameloblastic fibro-odontoma
│           ├── Adenomatoid odontogenic tumor (AOT)
│           ├── Calcifying cystic odontogenic tumor
│           ├── Cementoblastoma
│           ├── Odontoma
│               ├── Complex
│               └── Compound
│       └── Non-odontogenic tumors
│           └── Benign
│               ├── Osteoma
│               └── Chondroma
│           └── Malignant
│               ├── Osteosarcoma
│               └── Osteogenic secondary metastases
│       └── Osseous dysplasia
│           ├── Periapical osseous dysplasia
│           ├── Focal osseous dysplasia
│           ├── Florid osseous dysplasia
│           └── Familial gigantic form cementoma
│       └── Other lesions
│           ├── Ossyfying fibroma
│           └── Fibrous dysplasia
│       └── Bone diseases
│           ├── Paget s disease
│           └── Osteopetrosis
│       └── Soft tissue calcifications?
│           ├── Salivary calculi
│           ├── Calcified lymph nodes
│           ├── Calcified tonsils
│           ├── Phleoboliths
│           └── Calcified acne scars
│       └── Intrabony foregin bodies
│   └── Fractures
│       └── Maxillofacial fractures
│           ├── Le Fort I
│           ├── Le Fort II
│           └── Le Fort III
│       ├── Mandibular fracture
│       ├── Alveolar process fracture
│       └── Pathological fracture
│   └── Skeletal anomalies
│       └── Abnormalities of the maxilla
│           ├── Micrognathia
│           └── Macrognathia(maxillary prognathism)
│       └── Abnormalities of the mandible
│           ├── Micrognathia
│           ├── Macrognathia (mandibular prognathism)
│           ├── Condylar hypoplasia
│           ├── Condylar hyperplasia
│           ├── Bifid condyle
│           └── Coronoid hyperplasia
│       └── Clefts
│           └── Lip
│               └── Unilateral
│                   ├── With alveolar ridge
│                   └── Without alveolar ridge
│               └── Bilateral
│                   ├── With alveolar ridge
│                   └── Without alveolar rigde
│           └── Palate
│               ├── Soft palate only
│               ├── Soft and hard palate
│               └── Bifid uvula
│           └── Lip and palate
│               ├── Unilateral
│               └── Cleft palate with bilateral cleft lip
│       ├── Other rare developmental syndromes and diseases
│           ├── Cleidocranial dysplasia
│           ├── Gorlin's syndrome
│           ├── Eagle syndrome
│           ├── Crouzon syndrome (craniofacial dysostosis)
│           ├── Apert syndrome
│           └── Treacher Collins syndrome (mandibular facial dysostosis)
├── Salivary glands conditions
│   └── Anomalies
│       ├── Aplasia
│       ├── Atresia
│       └── Ectopia
│   └── Functional disorders
│       └── Xerostomia
│           ├── Primary
│           └── Secondary
│       └── Ptyalism
│   └── Obstructive and traumatic lesions
│       ├── Salivary duct fistula
│       ├── Sialolithiasis
│       └── Mucocele
│   └── Infections
│       └── Bacterial infections
│           ├── Acute sialoadenitis
│           └── Chronic sialoadenitis
│       └── Viral infections
│           └── Mumps
│       └── Allergic
│           └── Allergic sialoadenitis
│   └── Tumours
│       └── Malignant tumours
│           ├── Mucoepidermoid carcinoma
│           ├── Adenoid cystic carcinoma
│           ├── Actinic cell carcinoma
│           ├── Polymorphous adenocarcinoma
│           ├── Clear cell carcinoma
│           ├── Basal cell carcinoma
│           ├── Intraductal carcinoma
│           ├── Adenocarcinoma, NOS
│           ├── Salivary duct carcinoma
│           ├── Myoepithelial carcinoma
│           ├── Epithelial-myoepithelial carcinoma
│           ├── Carcinoma ex pleomofric adenoma
│           ├── Secretory carcinoma
│           ├── Sebaceous adenocarcinoma
│           ├── Carcinosarcoma
│           └── Poorly diferentiated carcinoma
│               ├── Undiferentiated carcinoma
│               ├── Large cell neuroendocrine carcinoma
│               └── Small cell neuroendocrine carcinomaq
│           ├── Lypmphoepithelial carcinoma
│           ├── Squamous cell carcinoma
│           ├── Oncotic carcinoma
│           ├── Uncertain malignant potential
│           └── Borderline tumour
│               └── Sialoblastoma
│       └── Benign tumours
│           ├── Pleomorfic adenoma
│           ├── Myoepithlioma
│           ├── Basal cell adenoma
│           ├── Warthin tumor
│           ├── Oncocytoma
│           ├── Lymphadenoma
│           ├── Cysstadenoma
│           ├── Sialoadenoma
│           ├── Ductal papillomas
│           ├── Sebaceous adenoma
│           ├── Canalicular adenoma
│           └── Canalicular adenoma and other ductal adenomas
│       └── Non-neoplastic epithelial lesions
│           ├── Sclerosing polycystic adenosis
│           ├── Nodular oncotic hyperplasia
│           ├── Lymphoepithelial sialadenitis
│           └── Intercalated duct hyperplasia
│       └── Benign soft tissue lesions
│           ├── Hemangioma
│           ├── Lipoma/sialolipoma
│           └── Nodular fasciitis
│       └── Hematolymphoid tumours
│           └── Extranodal marginal zone lymphoma of mucosa -
                associated lymphoid tissue (MALT lymphoma)

```

## Disclaimers

### 1. Ceratocysts...
